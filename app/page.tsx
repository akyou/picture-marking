...
function perspectiveRingPath(width: number, height: number, color: string, opacity: number, ringHeight = 37, ringPitch = 45) {
  ...
  const cx = width / 2
  const cy = height / 2
  const top = ellipsePath(cx, cy - d, R, ry)
  const bottom = ellipsePath(cx, cy + d, R, ry)
  const wall = `M ${cx - R} ${cy - d} A ${R} ${ry} 0 0 1 ${cx + R} ${cy - d} L ${cx + R} ${cy + d} A ${R} ${ry} 0 0 1 ${cx - R} ${cy + d} Z`
  return { path: `${top} ${bottom}`, wall, color, opacity }
}
