...
type Item =
  ...
  | { id: string; kind: 'stamp'; stamp: 'arrow' | 'circle' | 'check' | 'curve'; x: number; y: number; width: number; height: number; color: string; opacity: number; rotation: number; strokeWidth: number; arrowCurve?: number; arrowHeadSize?: number; arrowHeadAngle?: number; arrowTipInset?: number; locked?: boolean; hidden?: boolean }
...
function arrowGeometry(width: number, height: number, curve = 0, headSize = 38, headAngle = 28, tipInset = 20) {
...
export default function Page() {
  ...
  const addStamp = (stamp: Extract<Item, { kind: 'stamp' }>['stamp']) => {
    const item: Extract<Item, { kind: 'stamp' }> = { id: crypto.randomUUID(), kind: 'stamp', stamp, x: 340, y: 300, width: stamp === 'circle' ? 150 : 190, height: stamp === 'circle' ? 110 : 150, color, opacity: opacity / 100, rotation: 0, strokeWidth: 8, arrowCurve: 0, arrowHeadSize: 38, arrowHeadAngle: 28, arrowTipInset: 20 }
    commit([...items, item])
