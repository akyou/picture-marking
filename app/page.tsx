...
function perspectiveRingPath(width: number, height: number, color: string, opacity: number) {
  ...
  const d = (height / 2) * Math.sin(pitch)
  const top = ellipsePath(R, height / 2 - d, R, ry)
  const bottom = ellipsePath(R, height / 2 + d, R, ry)
  const wall = `M ${R - R} ${height / 2 - d} A ${R} ${ry} 0 0 1 ${R + R} ${height / 2 - d} L ${R + R} ${height / 2 + d} A ${R} ${ry} 0 0 1 ${R - R} ${height / 2 + d} Z`
  return { top, bottom, wall, color, opacity }
}
