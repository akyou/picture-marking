...
type AssetCategory = '表情' | '箭头' | '图标' | '常用'
type CustomAsset = { id: string; name: string; src: string; aspectRatio: number; category: AssetCategory }
const assetCategories: AssetCategory[] = ['常用', '表情', '箭头', '图标']
const textCharacters = ['①②③④⑤', '❶❷❸❹❺', '12345', '1️⃣2️⃣3️⃣4️⃣5️⃣', '★ ☆ ♥ ♡ → ← ↑ ↓ ✓ ✕']
type Item =
  | { id: string; kind: 'text'; x: number; y: number; text: string; size: number; color: string; weight: string; italic: boolean; underline: boolean; align: 'left' | 'center' | 'right'; lineHeight: number; letterSpacing: number; width: number; fontFamily?: string; locked?: boolean; hidden?: boolean }
...
export default function Page() {
  ...
  const [assetCategory, setAssetCategory] = useState<AssetCategory>('常用')
  ...
  useEffect(() => {
    ...
    if (storedAssets) {
      try { setCustomAssets(JSON.parse(storedAssets).map((asset: CustomAsset) => ({ ...asset, category: asset.category ?? '常用' }))) } catch { /* ignore invalid custom assets */ }
    }
  ...
  const readFile = (file: File, saveToLibrary = true) => {
    ...
    reader.onload = () => {
      ...
      image.onload = () => {
        ...
        if (saveToLibrary) setCustomAssets((assets) => [{ id: crypto.randomUUID(), name: file.name, src, aspectRatio, category: assetCategory }, ...assets.filter((asset) => asset.src !== src)])
  ...
  const alignSelected = (alignment: 'left' | 'center' | 'right' | 'top' | 'middle' | 'bottom') => {
    const targets = items.filter((item) => selectedIds.includes(item.id) && !item.locked && !item.hidden)
    if (targets.length < 2) return
    const bounds = targets.map(itemBounds)
    const left = Math.min(...bounds.map((bound) => bound.left))
    const right = Math.max(...bounds.map((bound) => bound.right))
    const top = Math.min(...bounds.map((bound) => bound.top))
    const bottom = Math.max(...bounds.map((bound) => bound.bottom))
    commit(items.map((item) => {
      if (!targets.some((target) => target.id === item.id) || item.kind === 'stroke') return item
      const bound = itemBounds(item)
      const x = alignment === 'left' ? left : alignment === 'center' ? (left + right - (bound.right - bound.left)) / 2 : alignment === 'right' ? right - (bound.right - bound.left) : item.x
      const y = alignment === 'top' ? top : alignment === 'middle' ? (top + bottom - (bound.bottom - bound.top)) / 2 : alignment === 'bottom' ? bottom - (bound.bottom - bound.top) : item.y
      return { ...item, x, y } as Item
    }))
  }
  ...
  const exportBoard = () => {
    ...
    items.forEach((item) => {
      if (item.kind === 'text') { ctx.fillStyle = item.color; ctx.font = `${item.italic ? 'italic ' : ''}${item.weight} ${item.size}px ${item.fontFamily ?? 'sans-serif'}`; ctx.textAlign = item.align; item.text.split('\\n').forEach((line, index) => ctx.fillText(line, item.x, item.y + index * item.size * item.lineHeight)); ctx.textAlign = 'left' }
    ...
    <main className="app-shell" onClick={() => contextMenu && setContextMenu(null)}>
      ...
      <div className="workspace">
        ...
        <section className="editor-area">
          <div className="editor-toolbar"><div className="tool-group"><button className="select-button"><MousePointer2 size={15} /> 选择 <ChevronDown size={14} /></button>{selectedIds.length >= 2 && <div className="alignment-tools" role="toolbar" aria-label="多图层对齐"><span className="alignment-label">对齐</span>{[['left', '左对齐'], ['center', '水平居中'], ['right', '右对齐'], ['top', '顶部对齐'], ['middle', '垂直居中'], ['bottom', '底部对齐']].map(([value, label]) => <button key={value} type="button" onClick={() => alignSelected(value as 'left' | 'center' | 'right' | 'top' | 'middle' | 'bottom')} title={label}>{label}</button>)}</div>}<span className="separator" /><button className="round-tool" onClick={undo} aria-label="撤销"><Undo2 size={17} /></button><div className="stamp-palette top-stamp-palette" aria-label="Apple 风格矢量标记"><span className="eyebrow">标记</span><button type="button" title="���头" aria-label="插入箭头" onClick={() => addStamp('arrow')}><ArrowUpRight size={18} strokeWidth={2.2} /></button><button type="button" title="圆圈" aria-label="插入圆圈" onClick={() => addStamp('circle')}><Circle size={18} strokeWidth={2.2} /></button><button type="button" title="勾画" aria-label="插入勾画" onClick={() => addStamp('check')}><Check size={19} strokeWidth={2.4} /></button><button type="button" title="直箭头" aria-label="插入直箭头" onClick={() => addStamp('arrow')}><ArrowUpRight size={18} strokeWidth={2.2} /></button></div><button className="round-tool" onClick={redo} aria-label="重做"><Redo2 size={17} /></button></div><div className="tool-group"><button className="zoom-button" onClick={() => setZoom(Math.max(50, zoom - 10))}><Minus size={14} /></button><span className="zoom-value">{zoom}%</span><button className="zoom-button" onClick={() => setZoom(Math.min(150, zoom + 10))}><Plus size={14} /></button><span className="separator" /><button className="export-button" onClick={exportBoard}><ArrowDownToLine size={15} /> 导出</button></div></div>
