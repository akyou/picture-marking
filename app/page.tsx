...
function perspectiveRingPath(width: number, height: number, color: string, opacity: number) {
  ...
  const outerRy = outerRx * Math.cos(pitch)
  const band = Math.max(4, Math.min(height * 0.22, 18))
  const innerRx = Math.max(1, outerRx - band)
  const innerRy = Math.max(1, outerRy - band * Math.cos(pitch))
  const cx = width / 2
  const cy = height / 2
