...
type Item =
  ...
  | { id: string; kind: 'stamp'; stamp: 'arrow' | 'circle' | 'check' | 'curve'; x: number; y: number; width: number; height: number; color: string; opacity: number; rotation: number; strokeWidth: number; arrowCurve?: number; arrowHeadSize?: number; arrowHeadAngle?: number; arrowHeadSharpness?: number; arrowTipInset?: number; locked?: boolean; hidden?: boolean }
...
function arrowGeometry(width: number, height: number, curve = 0, headSize = 38, headAngle = 28, headSharpness = 75, tipInset = 20) {
  const start = { x: 12, y: height - 24 }
  const tip = { x: width - tipInset, y: 24 }
  const dx = tip.x - start.x
  const dy = tip.y - start.y
  const length = Math.hypot(dx, dy) || 1
  const midpoint = { x: (start.x + tip.x) / 2, y: (start.y + tip.y) / 2 }
  ...
  const sharpnessFactor = 1.35 - (Math.max(10, Math.min(100, headSharpness)) / 100) * 0.95
...
export default function Page() {
  ...
  const addStamp = (stamp: Extract<Item, { kind: 'stamp' }>['stamp']) => {
    const item: Extract<Item, { kind: 'stamp' }> = { id: crypto.randomUUID(), kind: 'stamp', stamp, x: 340, y: 300, width: stamp === 'circle' ? 150 : 190, height: stamp === 'circle' ? 110 : 150, color, opacity: opacity / 100, rotation: 0, strokeWidth: 8, arrowCurve: 0, arrowHeadSize: 38, arrowHeadAngle: 28, arrowHeadSharpness: 75, arrowTipInset: 20 }
    commit([...items, item])
