...
type Item =
  | { id: string; kind: 'text'; x: number; y: number; text: string; size: number; color: string; weight: string }
...
const initialItems: Item[] = [
  { id: 'title', kind: 'text', x: 160, y: 120, text: '灵感画板', size: 42, color: '#17202a', weight: '700' },
  { id: 'note', kind: 'text', x: 164, y: 188, text: '把想法画出来', size: 20, color: '#718096', weight: '400' },
]
...
export default function Page() {
  ...
  const exportBoard = () => {
    ...
    items.forEach((item) => {
      if (item.kind === 'text') { ctx.fillStyle = item.color; ctx.font = `${item.weight} ${item.size}px sans-serif`; ctx.fillText(item.text, item.x, item.y) }
