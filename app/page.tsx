...
type Item =
  | { id: string; kind: 'text'; x: number; y: number; text: string; size: number; color: string; weight: string; italic: boolean; underline: boolean; align: 'left' | 'center' | 'right'; lineHeight: number; letterSpacing: number; width: number; locked?: boolean }
  | { id: string; kind: 'image'; x: number; y: number; width: number; height: number; src: string; name: string; aspectRatio: number; lockRatio: boolean; locked?: boolean }
  | { id: string; kind: 'stroke'; points: Point[]; color: string; width: number; opacity: number; rotation?: number; locked?: boolean }
  | { id: string; kind: 'stamp'; stamp: 'arrow' | 'circle' | 'check' | 'curve'; x: number; y: number; width: number; height: number; color: string; opacity: number; rotation: number; strokeWidth: number; arrowCurve?: number; locked?: boolean }
...
function arrowShaftPath(width: number, height: number, curve = 0) {
  const start = { x: 12, y: height - 24 }
  const end = { x: width - 20, y: 24 }
...
export default function Page() {
  ...
  const addStamp = (stamp: Extract<Item, { kind: 'stamp' }>['stamp']) => {
    const item: Extract<Item, { kind: 'stamp' }> = { id: crypto.randomUUID(), kind: 'stamp', stamp, x: 340, y: 300, width: stamp === 'circle' ? 150 : 190, height: stamp === 'circle' ? 110 : 150, color, opacity: opacity / 100, rotation: 0, strokeWidth: 8, arrowCurve: 0 }
    commit([...items, item])
    setSelectedId(item.id)
